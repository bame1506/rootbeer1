...
public final class ReverseClassHierarchy
{
    ...
    )
    {
        ...
        for ( final Integer root : roots )
        {
            ...
            OpenCLClass ocl_class = scene.getOpenCLClass(root_class);
            ...
            while ( ! queue.isEmpty() )
            {
                ...
                for(Integer child : children)
                {
                    ...
                    if ( child_class.isInterface() )
                        continue;
                    if ( ! dfs_string_types.contains(child_str) )
                        continue;
                    OpenCLClass ocl_class2 = scene.getOpenCLClass(child_class);
        ...
        for ( final TreeNode tree_node : all_tree_nodes )
        {
    ...
    public List<TreeNode> get(){ return m_hierarchy; }

    private Set<String> getRefTypes( final Set<Type> dfs_types )
    {
        final Set<String> ret = new HashSet<String>();
        for ( final Type type : dfs_types )
        {
    ...
    private boolean hasNewInvoke( final TreeNode tree_node, final Set<String> dfs_types )
    {
